...
use Notadd\Foundation\JWTAuth\Exceptions\PayloadException;
...
class Payload implements ArrayAccess, Arrayable, Countable, Jsonable, JsonSerializable
{
     ...
     * @param mixed  $value
     *
     * @throws \Notadd\Foundation\JWTAuth\Exceptions\PayloadException
     */
    ...
    public function __call($method, $parameters)
    {
        if (preg_match('/get(.+)\b/i', $method, $matches)) {
            foreach ($this->claims as $claim) {
                if (get_class($claim) === 'Notadd\\Foundation\\JWTAuth\\Claims\\'.$matches[1]) {
